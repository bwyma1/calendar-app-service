...
public func eventMatchesFilters(_ event: NOSTR_event_signed<UnsignedEvent<CalendarEventContent>>, filters: [Filter], logLevel: Logger.Level) -> Bool {
	...
	for filter in filters {
		...
		if((filter.ids == [] || filter.ids.contains(event.unsignedEvent.id)) &&
		   ...
		   (filter.since == nil || filter.since!.currentDate() <= event.unsignedEvent.content.start.toDate()) &&
		   (filter.until == nil || filter.until!.currentDate() >= event.unsignedEvent.content.start.toDate()) &&
		   hasTags) {
...
@inline(__always)
public func eventMatchesFilters(_ event: NOSTR_event_signed<UnsignedEvent<MemberContent>>, filters: [Filter], logLevel: Logger.Level) -> Bool {
	var logger = Logger(label: "filter")
	logger.logLevel = logLevel
	guard !filters.isEmpty else {
		return true
...
public struct EventDB:Sendable {
	...
	fileprivate let calendarEvents:Database.Strict<NOSTR_id, NOSTR_event_signed<UnsignedEvent<CalendarEventContent>>>
	fileprivate let memberEvents:Database.Strict<NOSTR_id, NOSTR_event_signed<UnsignedEvent<MemberContent>>>
	...
	public init(base:Path, logLevel:Logger.Level) throws {
		...
		env = try Environment(path:finalPath.path(), flags:[.noSubDir], mapSize:memoryMapSize, maxReaders:8, maxDBs:2, mode:[.ownerReadWriteExecute, .groupReadExecute, .otherReadExecute])
		...
		calendarEvents = try Database.Strict<NOSTR_id, NOSTR_event_signed<UnsignedEvent<CalendarEventContent>>>(env:env, name:"CalendarDB", flags:[.create], tx:newTrans)
		log.trace("created main database. now committing transaction", metadata:["path":"\(finalPath.path())", "memoryMapSize":"\(memoryMapSize)b"])
		memberEvents = try Database.Strict<NOSTR_id, NOSTR_event_signed<UnsignedEvent<MemberContent>>>(env:env, name:"MemberDB", flags:[.create], tx:newTrans)
		try newTrans.commit()
	}
	
	public func clearDatabase() throws {
		let newTrans = try Transaction(env: env, readOnly: false)
		try calendarEvents.deleteAllEntries(tx: newTrans)
		try memberEvents.deleteAllEntries(tx: newTrans)
		try newTrans.commit()
		log.info("successfully deleted events database")
	}

	/// Scribes a new (already Signature Confirmed!!!) calendar event into the events database.
	public func scribeNewCalendarEvent(signedEvent:NOSTR_event_signed<UnsignedEvent<CalendarEventContent>>, logLevel:Logger.Level) throws {
		var logger = log
		logger.logLevel = logLevel
		...
		let newTrans = try Transaction(env:env, readOnly:false)
		try calendarEvents.cursor(tx:newTrans) { cursor in
			do {
	...
	/// Scribes a new (already Signature Confirmed!!!) member event into the events database.
	public func scribeNewMemberEvent(signedEvent:NOSTR_event_signed<UnsignedEvent<MemberContent>>, logLevel:Logger.Level) throws {
		var logger = log
		logger.logLevel = logLevel
		logger[metadataKey:"event_id"] = "\(String(describing: signedEvent.unsignedEvent.id))"
		...
		logger.trace("opening transaction to write data to events database")
		let newTrans = try Transaction(env:env, readOnly:false)
		try memberEvents.cursor(tx:newTrans) { cursor in
			do {
				guard try cursor.containsEntry(key:signedEvent.unsignedEvent.id) == false else {
					logger.info("Attempted to write an event that already exists")
					throw LMDBError.keyExists
	...
	public func filterCalendarEvents(filter:[Filter]) throws -> [NOSTR_event_signed<UnsignedEvent<CalendarEventContent>>] {
	...
	/// Follows the NOSTR protocol where the filters use an OR function and each filter's fields use an AND function.
	/// Thus, a signed event only needs to match all of the constraints of a single filter to be returned.
	public func filterMemberEvents(filter:[Filter]) throws -> [NOSTR_event_signed<UnsignedEvent<MemberContent>>] {
		var filteredEvents:[NOSTR_event_signed<UnsignedEvent<MemberContent>>] = []
		// Filters are OR'd and filter fields are AND'ed
		let newTrans = try Transaction(env:env, readOnly:true)
		do {
			try memberEvents.cursor(tx:newTrans) { cursor in
